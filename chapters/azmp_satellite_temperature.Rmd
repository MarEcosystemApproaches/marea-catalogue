...
```{r plot-azmp-satellite-temperature, fig.cap=paste0("AZMP Satellite Surface Temperature in Scotian Shelf Regions; ",marea_metadata() %>% filter(Dataset == indicator) %>% pull(TimeSpan)), echo=T}
plotly_df <- data@data %>%
...
Surface temperatures are less variable across the region compared to bottom temperature. Among regions in this dataset, mean surface temperatures range from **`r highest_mean`°C in `r highest_region`** to **`r lowest_mean`°C in `r lowest_region`**. `r last10$last10_high` regions had their highest temperature values within the last 10 years of sampling (`r last_year-10`-`r last_year`), and `r last10$last10_low` had their lowest value (Fig. \@ref(fig:plot-azmp-satellite-temperature)).
...
```{r plot-trends-azmp-satellite-temperature, fig.cap="Coefficients of satellite surface temperature trends over time for NAFO zones within Eastern and Western Scotian Shelf regions.", echo=F}
slopes_p2
```


The overall trend is **`r majority_trend`** across regions. `r n_increasing` regional subdivisions have increasing Bottom Temperature trends (all of which are statistically significant) and `r n_decreasing` have negative trends (Fig \@ref(fig:plot-trends-azmp-satellite-temperature)).

As with Bottom Temperature, average satellite surface temperature exhibited a general northward decrease between NAFO zones (Fig \@ref(fig:plot-trends-azmp-satellite-temperature-map)), but warming trends were fairly consistent across zones (Fig. \@ref(fig:plot-trends-azmp-satellite-temperature),\@ref(fig:plot-trends-azmp-satellite-temperature-map)).

```{r plot-trends-azmp-satellite-temperature-map, fig.cap="Map of target study areas colored by mean satellite surface temperature (left) and warming trend over time (right).", echo=F}
library(sf)
library(mapgl)
...
Summary values for the AZMP Satellite Temperature within each individual NAFO zone in the Eastern and Western Scotian Shelf are shown below (Table \@ref(tab:azmp-satellite-temperature-trends-table))

Table: (\#tab:azmp-satellite-temperature-trends-table) Summary values of AZMP Satellite Temperature for NAFO zones in the Eastern Scotian Shelf (4Vn, 4Vs, 4W), and Western Scotian Shelf (4X) values through time. For warming trends, asterisks indicate significant trends over time (p < 0.05). 
