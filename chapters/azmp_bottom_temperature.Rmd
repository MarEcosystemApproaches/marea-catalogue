...
Temperature data from continuous downcast CTD profiles are interpolated to a defined grid using an objective analysis procedure, and results are spatially weighted for NAFO divisions 4Vn, 4Vs, 4W, and 4X [@canada2025azmp].


<!----------- Plots Section -------------->
## View Data
<!-- USE DEFAULT MAREA PLOT OR REPLACE WITH CUSTOM -->
```{r plot-azmp-bottom-temperature, fig.cap=paste0("AZMP Regional Bottom Temperature in Scotian Shelf Regions; ", timespan), echo=TRUE}
library(plotly)
...
In the most recent year of sampling (`r last_year`), **`r add_s(last_percentile_groups[["n_high"]],"region")` had values that are high** compared to their overall timeseries (>70th percentile), **`r add_s(last_percentile_groups[["n_mid"]],"region")` had values that were moderate** (between 30th and 70th percentile), and **`r add_s(last_percentile_groups[["n_low"]],"region")` had low values** (<30th percentile) within their timeseries.


```{r plot-trends-azmp-bottom-temperature, fig.cap=paste0("Model coefficients of bottom temperature over time for NAFO divisions in the Eastern and Western Scotian Shelf Regions; ",timespan, ". Error bars represent 95% confidence intervals of model fits. Asterisks indicate significant trends (p value of year coefficient < 0.05)."), echo=F}
slopes_p2
```



The overall trend is **`r majority_trend`** across regions. `r add_s(n_increasing,"regional subdivision")` had increasing Bottom Temperature trends (all of which are statistically significant) and `r n_decreasing` had negative trends (Fig \@ref(fig:plot-trends-azmp-bottom-temperature)).

Average bottom temperature exhibited a general northward decrease between NAFO divisions (Fig \@ref(fig:plot-trends-azmp-bottom-temperature-map)), but warming trends were fairly consistent across zones (Fig. \@ref(fig:plot-trends-azmp-bottom-temperature),\@ref(fig:plot-trends-azmp-bottom-temperature-map)).

```{r plot-trends-azmp-bottom-temperature-map,fig.cap="Map of target study areas colored by mean temperature (left) and warming trend over time (right).", echo=F, cache=F}
library(sf)
library(mapgl)
...
Summary values for the AZMP Bottom Temperature within each individual NAFO division in the Eastern and Western Scotian Shelf are shown below (Table \@ref(tab:azmp-bottom-temperature-trends-table))

Table: (\#tab:azmp-bottom-temperature-trends-table) Summary values of AZMP Bottom Temperature for NAFO division in the Eastern Scotian Shelf (4Vn, 4Vs, 4W), and Western Scotian Shelf (4X) values through time. For warming trends, asterisks indicate significant trends over time.

|Metric      |Eastern Scotian Shelf     |Western Scotian Shelf  |
|:-----------|:------------------|:------------------|
| Mean Value (±SE) |`r ess_means`      |`r wss_means`      |
...
In the Scotian Shelf-Bay of Fundy region, warming bottom temperatures have been associated with negative impacts and/or northward distribution shifts to key decapod species, northern shrimp and snow crab, for which this region represents the southernmost extent of both fisheries [@zisserson2017impact; @chang2025temperature]. Alternatively, recent increases in bottom temperature across the Northwestern Atlantic have increased suitable thermal habitat for halibut in most Eastern Canadian NAFO divisons, and models project further increases in abundance within the Canadian range as bottom temperatures continue to increase [@shackell2022spatial; @czich2023recent].
