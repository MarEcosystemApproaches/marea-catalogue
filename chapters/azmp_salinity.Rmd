...
Monthly mean near surface salinity values are calculated for hydrographic areas 4 to 23 defined by [@petrie1996]. An annual value is calculated for each area by taking the mean of all available months, then an area-weighted mean is calculated to get a Scotian Shelf value [@canada2025azmp].
...
```{r plot-azmp-salinity, fig.cap=paste0("AZMP Salinity in Scotian Shelf Regions; ",timespan), echo=TRUE}
library(plotly)
plotly_df <- data@data %>%
  mutate( 
...
streak_start <- switch(current_vs_mean_text,
...
<!---------- Trends Text ----------->
As of the most recent data entry in **`r last_date`**, the Surface salinity value is `r last_val` PSU, which is **`r current_magnitude`** among values in the timeseries.
Surface salinity is currently **`r current_vs_mean_text` the long-term mean**, and has been **since `r streak_start_date`**.
The regional salinity value has followed **`r current_trend` trend** in recent years (Fig \@ref(fig:plot-azmp-salinity)).
...
Summary values for the AZMP Salinity in the Scotian Shelf region are shown below (Table \@ref(tab:azmp-salinity-trends-table))

Table: (\#tab:azmp-salinity-trends-table) Summary values of AZMP Salinity in the Scotian Shelf region through time. 
...
Table: (\#tab:azmp-salinity-variable-definitions) Column names and definitions in the AZMP Surface Salinity dataset.
