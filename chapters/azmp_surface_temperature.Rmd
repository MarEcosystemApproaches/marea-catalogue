...
```{r plot-azmp-surface-temperature, fig.cap=paste0("AZMP Surface Temperature in Scotian Shelf; ", timespan), echo=TRUE}
library(plotly)
plotly_df <- data@data %>%
  mutate( 
...
Surface temperature has followed **`r current_trend` trend** in recent years (Fig. \@ref(fig:plot-azmp-surface-temperature)).
...
Table: (\#tab:azmp-surface-temperature-trends-table) Summary values of AZMP Surface Temperature in the Scotian Shelf region through time. 
